...
class tlAdapter():
    ...
    def drive_input(self, memory):
        assert memory.__class__.__name__ == 'dict' or IS_DISCO, \
        ...
        while self.drive:
            if a_ports.fire():
                ...
                if opcode == PUT_FULL_DATA:
                    ...
                    if addr == 0x60000000 and IS_DISCO:
                        print('[Disco] Found stop request', flush=True)
    ...
    def host_interface(self, block_perm, b_srcs, b_callback):
        ...
        while self.drive:
            if (self.probe & self.probe_en) and self.probe_addr not in self.ongoing_tlc.values():
                ...
                if block_perm[block_addr] != TIP or IS_DISCO:
